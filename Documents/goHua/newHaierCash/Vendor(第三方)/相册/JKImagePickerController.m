...
@interface JKImagePickerController ()<JKAssetsGroupsViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,JKAssetsViewCellDelegate,JKPhotoBrowserDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,DSComposePhotosViewDelegate>
...
@property(nonatomic,copy)NSString *killJack;
...
@property (nonatomic , strong) DSComposePhotosView *photosView;

@property(nonatomic,assign)NSInteger selectIndex;//记录当前点击次数
...
- (void)viewDidLoad {
    ...
    self.tempArray = [NSMutableArray arrayWithArray:self.selectedAssetArray];
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    self.photosView.assetsArray = self.tempArray;
    self.selectedAssetArray = self.tempArray;
}
...
{
    JKPhotoBrowser  *photoBorwser = [[JKPhotoBrowser alloc] initWithFrame:[UIScreen mainScreen].bounds isSelect:YES];
...
//        cell.asset = nil;
//    }else{
        ALAsset *asset = self.assetsArray[self.assetsArray.count-(indexPath.row+1)];
        cell.asset = asset;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_selectIndex<=1) {
        ...
        [self browerPhotoes:self.assetsArray page:self.assetsArray.count-(indexPath.row+1)];
...
-(void)sendBrowser:(JKPhotoBrowser *)photoBrowser didSelectAtIndex:(NSInteger)index{
       ...
       WEAKSELF
            [lib assetForURL:assetURL resultBlock:^(ALAsset *myasset){
                ...
                if (myasset) {
                    ...
                    if ([strongSelf.killJack isEqualToString:@"YES"]) {
...
- (UIButton *)arrowImageView{
    if (!_arrowImageView) {
        UIImage  *img = [UIImage imageNamed:@"drop-down"];
        UIImage  *imgSelect = [UIImage imageNamed:@"oil_drop_up"];
        _arrowImageView = [UIButton buttonWithType:UIButtonTypeCustom];
        _arrowImageView.frame = CGRectMake(0, 5, img.size.width, img.size.height);
...
//        [_finishButton setBackgroundImage:[JKUtil stretchImage:[UIImage imageNamed:@"picker_button_orange_highlighted"] capInsets:UIEdgeInsetsMake(5, 5, 5, 5) resizingMode:UIImageResizingModeStretch] forState:UIControlStateHighlighted];
        ...
        _finishButton.backgroundColor = [UIColor UIColorWithHexColorString:@"0x32beff" AndAlpha:1.0];
