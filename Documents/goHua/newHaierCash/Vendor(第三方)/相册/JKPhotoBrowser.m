...
- (id)initWithFrame:(CGRect)frame isSelect:(BOOL)isSelect1
{
    self = [super initWithFrame:frame];
    if (self) {
        self.isSelect = isSelect1;
...
- (void)photoDidChecked
{
    if (!self.checkButton.selected&&self.pickerController.selectedAssetArray.count>=self.pickerController.maximumNumberOfSelection) {
        NSString  *str = [NSString stringWithFormat:@"最多选择%lu张照片",(unsigned long)self.pickerController.maximumNumberOfSelection];
    ...
    if (animated){
        [UIView animateWithDuration:0.3f
                         animations:^{
                             ...
                             strongSelf.top = [UIScreen mainScreen].bounds.size.height;
                         }
                         completion:^(BOOL finished) {
                             
                             STRONGSELF
                             
                             [strongSelf.delegate sendHidden];
...
-(UIButton *)chooseBtn{
    ...
    if (!_chooseBtn) {
        ...
        _chooseBtn = [[UIButton alloc]initWithFrame:CGRectMake(DeviceWidth-80, DeviceHeight-40, 80, 40)];
        
        [_chooseBtn setTitle:@"确定" forState:UIControlStateNormal];
        
        _chooseBtn.backgroundColor = [UIColor UIColorWithHexColorString:@"0x32beff" AndAlpha:1.0];
        
        [_chooseBtn addTarget:self action:@selector(buildSureAction:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)buildSureAction:(UIButton *)sender{
    
    [self hide:YES];
    
    if (_delegate && [_delegate respondsToSelector:@selector(sendBrowser:didSelectAtIndex:)]) {
        
        [_delegate sendBrowser:self didSelectAtIndex:self.currentPage];
