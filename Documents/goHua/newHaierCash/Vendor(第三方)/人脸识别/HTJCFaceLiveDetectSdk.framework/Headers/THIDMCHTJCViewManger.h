 ...
 *      _failed：失败返回值
 */
-(void)getLiveDetectCompletion:(void (^)(BOOL success,NSData * imageData))_completion  cancel:(void(^)(BOOL success, NSString* error))_cancel failed:(void (^)(NSString *error))_failed;
 ...
 *  活体检测动作配置
 *
 *  @param liveDetectTypeArray   数组 @0 = 凝视  @1 = 摇头  @2 = 点头
 */
