...
-(void)createNavigation{
    ...
    navigationView.backgroundColor = NAVIGATION_COLOR;
...
-(void)createView{
    _changedView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREENWIDTH, SCREENHEIGHT)];
    _changedView.backgroundColor = [DefineSystemTool naviColor];
...
-(void)changeToFailView:(NSString *)failedStr{
    [_changedView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    [self createFailedView:failedStr];
}
...
-(void)creatBeginView{
    ...
    beginBtn.frame = CGRectMake(SCREENWIDTH/12, CHANGEVIEW_HEIGHT/13*11, SCREENWIDTH/12*10, 40);
    ...
    label.text = @"应用版本号:V1.2.3  算法版本号:S1.0.1.4126";
...
//失败view
-(void)createFailedView:(NSString *)failedStr{
    _titleLB.text = @"检测结果";
    
    UIImageView * photoImgView = [[UIImageView alloc]initWithFrame:CGRectMake(SCREENWIDTH/4, CHANGEVIEW_HEIGHT/13, SCREENWIDTH/2, CHANGEVIEW_HEIGHT/13*3)];
    photoImgView.image = [UIImage imageNamed:@"HTJCData.bundle/failedImage.png"];
    [_changedView addSubview:photoImgView];
    
    UILabel *failLB = [[UILabel alloc]initWithFrame:CGRectMake(SCREENWIDTH/4, CHANGEVIEW_HEIGHT/13*4, SCREENWIDTH/2, 50)];
    failLB.text = @"检测失败";
    ...
    //
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(SCREENWIDTH/11, CHANGEVIEW_HEIGHT/13*6, SCREENWIDTH/11*9, 40)];
    label.text = @"可能失败的原因:";
    ...
    CGSize size = [failedStr boundingRectWithSize:CGSizeMake(SCREENWIDTH/9*7, 10000) options:\
                   NSStringDrawingTruncatesLastVisibleLine |
                   NSStringDrawingUsesLineFragmentOrigin |
                   NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:17.0f]} context:nil].size;
    //取得高度
    CGFloat h = size.height;
    UILabel *reasonLabel = [[UILabel alloc]initWithFrame:CGRectMake(SCREENWIDTH/11, CHANGEVIEW_HEIGHT/13*6+40, SCREENWIDTH/11*9, h)];
