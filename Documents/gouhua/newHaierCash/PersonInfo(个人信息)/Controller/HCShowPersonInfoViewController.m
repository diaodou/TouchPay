...
-(void)buildPhoto{
    ...
    if (![DefineSystemTool isGetPhotoPermission]) {
        ...
        //改变导航栏背景色f
        [[UINavigationBar appearance] setBarTintColor:[UIColor UIColorWithHexColorString:@"0x028ce5" AndAlpha:1.0]];
    ...
    if (_checkMsgModel) {
        ...
        if ([AppDelegate delegate].userInfo.busFlowName == CashLoanCreate || [AppDelegate delegate].userInfo.busFlowName == CashLoanWait|| [AppDelegate delegate].userInfo.busFlowName == CashLoanReturned){
            ...
            if (_typCde.length > 0 && _typCde) {
                ...
                }else if ([AppDelegate delegate].userInfo.busFlowName == GoodsLoanCreate || [AppDelegate delegate].userInfo.busFlowName == GoodsLoanWait || [AppDelegate delegate].userInfo.busFlowName == GoodsLoanReturned ){
...
-(void)buildHandleSaveInfo:(PortraitImageModel*)model{
    ...
    if ([model.head.retFlag isEqualToString:@"00000"]) {
#pragma mark -->暂时注释
        //        [AppDelegate delegate].userInfo.bPerfect = YES;
        
        [self buildSaveContactInfo];
        
    }else{
#pragma mark -->暂时注释
        //[AppDelegate delegate].userInfo.bPerfect = NO;
